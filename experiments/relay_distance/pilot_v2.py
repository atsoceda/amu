#!/usr/bin/env python3
"""Relay-at-a-distance pilot v2 (design frozen 2026-09-26, README "Pilot v2").
...
def main() -> None:
    ...
    ck = Checkpoint(EXP / "results" / a.model / "pilot_v2_rows.json", key=lambda r: f"{r['idx']}-{r['distance']}")
    ...
    for idx, r in df.iterrows():
        ...
        for D in dists:
            ...
            rec = {"idx": int(idx), "distance": D, "n_tokens": int(ids.shape[1]), "line2": line2,
                   ...
                   **{f"relay_{g}": (R(run(ids, on_g[g])[0]) - base) if g in on_g else 0.0 for g in groups}}
            del s_off, s_on
            ck.add(rec)
            print(f"{len(ck.rows):3d} [{time.time()-t0:5.0f}s] D={D:5d} T={rec['n_tokens']:5d} pers {rec['persistence']:+.2f} "
                  f"ret {rec['retrieval']:+.2f} relay {rec['relay']:+.2f} (boundary {rec['relay_boundary']:+.2f} filler {rec['relay_filler']:+.2f} line2 {rec['relay_line2']:+.2f}) | {line2[:50]!r}", flush=True)
    rows = ck.rows
    ...
    (out / "pilot_v2_summary.json").write_text(json.dumps(s, indent=1))
