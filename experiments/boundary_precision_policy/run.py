...
def main() -> None:
    ...
    for index, example in enumerate(e1["test_examples"], start=1):
        ...
        if index not in set(config["dense_prompt_indices"]):
            continue
        precision_cache: dict[tuple[str, float], tuple[torch.Tensor, torch.Tensor]] = {}

        def precision(prefix: str, gain: float) -> tuple[torch.Tensor, torch.Tensor]:
            nonlocal audit
            key = (prefix, round(float(gain), 12))
            if key not in precision_cache:
                payload = feature_intervention_precision(model, prefix, intervention_tuples(gain))
                if audit is None:
                    audit = dtype_audit(model, payload)
                    audit["recorded_feature_cache"] = str(activations.dtype)
                precision_cache[key] = (vector(payload["native_logits"]), vector(payload["float32_logits"]))
            return precision_cache[key]

        search_half = float(config["float32_search_half_width"])
        fp_low_gain = max(float(config["gain_low"]), center - search_half)
        fp_high_gain = min(float(config["gain_high"]), center + search_half)
        _, fp_low_logits = precision(prompt, fp_low_gain)
        _, fp_high_logits = precision(prompt, fp_high_gain)
        fp_low_margin = float(fp_low_logits[an_id] - fp_low_logits[a_id])
        fp_high_margin = float(fp_high_logits[an_id] - fp_high_logits[a_id])
        if not fp_low_margin <= 0 < fp_high_margin:
            raise RuntimeError(f"Float32 boundary not bracketed for prompt {index}: {fp_low_margin}, {fp_high_margin}")
        for _ in range(int(config["float32_search_steps"])):
            fp_mid_gain = (fp_low_gain + fp_high_gain) / 2
            _, fp_mid_logits = precision(prompt, fp_mid_gain)
            fp_mid_margin = float(fp_mid_logits[an_id] - fp_mid_logits[a_id])
            if fp_mid_margin > 0:
                fp_high_gain, fp_high_margin = fp_mid_gain, fp_mid_margin
            else:
                fp_low_gain, fp_low_margin = fp_mid_gain, fp_mid_margin
        fp_center = (fp_low_gain + fp_high_gain) / 2
        ...
        for grid_index in range(count):
            ...
            gain = fp_center + offset
            native_logits, fp32_logits = precision(prompt, gain)
        ...
        for side, gain in (("low", fp_low_gain), ("high", fp_high_gain)):
            article_native, article_fp32 = precision(prompt, gain)
